...
| Account-state inspector | Inspect margin mode, position mode, collateral, maintenance margin, liquidation distance, unrealized PnL, funding exposure, open orders, and hedge books | `GetExchangeState`, `GetAccountState` | Auth, local/mock source label, data freshness, no execution |
| Symbol metadata inspector | Inspect contract status, role, tick size, lot size, min quantity, max quantity, min notional, order types, and time-in-force | `GetSymbolMetadata` | Auth, source timestamp, stale metadata labeling |
| Fee and symbol policy inspector | Inspect maker fee, taker fee, source, freshness, promotion metadata, fallback fee, and executable/derived/reference symbol roles | `GetFeePolicy`, `GetSymbolUniverse` | Auth, stale fee labeling, no permanent zero-fee assumption |
